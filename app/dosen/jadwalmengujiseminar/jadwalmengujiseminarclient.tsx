...
import { 
  ...
  MessageSquare, AlertCircle, ShieldCheck, Lock,
  Calculator
} from "lucide-react";
...
// ================= FETCHER SWR =================
const fetchDetailUjianSeminarDosen = async (id: string | null) => {
  if (!id) return null;
...
function DetailContent() {
  ...
  const { data: cache, isLoading, mutate } = useSWR(
    id ? `jadwal_menguji_seminar_dosen_${id}` : null,
    () => fetchDetailUjianSeminarDosen(id),
    {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push('/dosen/jadwalpengujiseminar'); 
    } catch (err: any) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-white p-8 rounded-[2rem] shadow-xl shadow-slate-200/40 border border-white flex flex-col items-center text-center">
              ...
              <h2 className="text-lg font-black text-slate-800 capitalize tracking-tight leading-tight mb-1">
              ...
              <div className="w-full text-center">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Dokumen Skripsi Mahasiswa</p>
                {draftUrl ? (
