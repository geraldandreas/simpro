...
const fetchJadwalSidangDosen = async () => {
  ...
  // 1. Fetch jadwal Sidang (Hanya di mana dosen menjadi PENGUJI)
  let query = supabase
  ...
  const mappedData = myRequests.map((reqDetail: any) => {
    ...
    return {
      ...
      role_saya: "Penguji" // Pasti penguji karena sudah di filter dari awal
    };
  });
                 ...
                 <tbody className="divide-y divide-slate-50">
                   {isLoading ? (
                     ...
                     <tr key={idx} className="group hover:bg-blue-50/30 transition-all duration-300">
                       ...
                       <td className="py-8 px-8 text-center">
                         <button 
                           onClick={() => router.push(`/dosen/penilaiansidang?id=${item.id_request}`)}
