...
const fetchJadwalSeminarData = async (requestId: string | null) => {
  ...
  // 4. Fetch Existing Schedule & Examiners
  ...
  const handleTetapkanJadwal = async () => {
    ...
    try {
      ...
      examinerIds.forEach(pengujiId => {
        if (pengujiId) {
          notificationPromises.push(sendNotification(pengujiId, "Tugas Menguji Seminar", `Kaprodi telah menetapkan jadwal Anda untuk menguji seminar mahasiswa atas nama ${mhsName} pada ${formattedDate}, pukul ${jam} WIB di ${ruangan}.`));
        }
      });
