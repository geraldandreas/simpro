...
// ================= FETCHER SWR =================
const fetchProgresSemuaMahasiswa = async () => {
  ...
  const mappedData: StudentProgress[] = (proposals || [])
    .filter((p: any) => p.status !== "Lulus") // Menyembunyikan yang sudah lulus
    .map((p: any) => {
      // --- LOGIKA SINKRONISASI (Disamakan persis dengan Tendik) ---
      ...
      // 🔥 LOGIKA OVERRIDE STATUS (Disamakan persis dengan Tendik) 🔥
      ...
      if (p.status_lulus === true || p.status === 'Lulus') {
        ...
        if (hasSidang) {
           ...
           // 🔥 TAHAN STATUS JIKA BELUM DIJADWALKAN OLEH KAPRODI 🔥
           else if (sidangStatus === "menunggu_penjadwalan" || sidangStatus === "pending") {
             finalTahap = "Pendaftaran Sidang Skripsi";
           } 
           // Jika sudah dijadwalkan, disetujui, atau selesai, barulah masuk ke tahap Sidang Skripsi
           else {
...
export default function ProgresSemuaMahasiswaKaprodiClient() {
  ...
  // ================= HELPER WARNA BADGE (Disamakan persis dengan Tendik) =================
    ...
    <div className="flex flex-col w-full h-full bg-[#F8F9FB] font-sans text-slate-700">
      <main className="flex-1 p-10 overflow-y-auto outline-none focus:outline-none">
        <div className="mb-10">
          ...
          <p className="text-slate-500 font-medium mt-1">Daftar Mahasiswa Aktif dalam pengerjaan tugas akhir.</p>
        </div>
        ...
        <div className="bg-white rounded-[2rem] border border-white shadow-xl shadow-slate-200/50 overflow-hidden outline-none focus:outline-none">
          <div className="p-8 flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-slate-50 bg-slate-50/30">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-black text-slate-800 uppercase tracking-tighter">Mahasiswa Aktif</h2>
            </div>
            ...
            <div className="flex flex-wrap gap-4">
              <div className="relative group">
                ...
                <select 
                  ...
                  className="bg-white text-slate-700 text-sm font-black border border-slate-200 rounded-2xl pl-12 pr-10 py-3 focus:ring-4 focus:ring-blue-50 focus:border-blue-400 cursor-pointer appearance-none transition-all min-w-[240px] shadow-sm outline-none"
                >
            ...
            <table className="w-full text-left border-collapse outline-none focus:outline-none">
             <thead>
                <tr className="bg-slate-50/50 text-[11px] uppercase tracking-[0.15em] text-slate-400 font-black border-b border-slate-100">
                  ...
                  <th className="px-8 py-6 text-center w-[10%]">Aksi</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50 outline-none focus:outline-none">
                {isLoading ? (
                  <>
                    {[1, 2, 3, 4, 5].map((item) => (
                      <tr key={item} className="border-b border-slate-50 outline-none focus:outline-none animate-pulse">
                        <td className="px-8 py-8"><div className="flex items-center gap-4"><div className="w-10 h-10 rounded-xl bg-slate-200"></div><div className="h-3 w-32 bg-slate-200 rounded-full"></div></div></td>
                        <td className="px-8 py-8"><div className="h-2.5 w-3/4 bg-slate-200 rounded-full mx-auto"></div></td>
                        <td className="px-8 py-8"><div className="h-6 w-24 bg-slate-200 rounded-xl mx-auto"></div></td>
                        <td className="px-8 py-8"><div className="h-2.5 w-28 bg-slate-200 rounded-full mx-auto"></div></td>
                        <td className="px-8 py-8 text-center"><div className="h-9 w-24 bg-slate-200 rounded-xl mx-auto"></div></td>
                      </tr>
                    ))}
                  </>
                  ...
                  <tr>
                    <td colSpan={5} className="px-8 py-24 text-center">
                       <div className="flex flex-col items-center gap-3">
                          ...
                          <p className="text-slate-400 font-black uppercase tracking-widest text-sm">Tidak Ada Mahasiswa Aktif</p>
                       </div>
                    </td>
                  </tr>
                ) : (
                  currentData.map((mhs) => (
                    <tr key={mhs.id} className="group hover:bg-blue-50/30 transition-all duration-300 outline-none focus:outline-none">
                      ...
                      <td className="px-8 py-8 text-center">
                        <p className="text-[13px] font-bold text-slate-600 leading-relaxed line-clamp-2 px-6">"{mhs.judul}"</p>
                      </td>
                      <td className="px-8 py-8 text-center">
                     ...
                     <td className="px-8 py-8 text-center">
                        <Link href={`/kaprodi/detailprogres?id=${mhs.id}`}>
                          <button className="group/btn inline-flex items-center gap-2 px-5 py-2.5 bg-slate-900 hover:bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all shadow-lg active:scale-95 outline-none focus:outline-none focus:ring-0">
