...
export default function JadwalPengujiKaprodiClient() {
  ...
  const fetchSchedules = async () => {
    ...
    try {
      ...
      // Pertama cari proposal di mana dia jadi pembimbing
      const { data: supervisorAssignments } = await supabase
        .from('thesis_supervisors')
        .select('proposal_id')
        .eq('dosen_id', user.id);

      // Ambil semua ID Seminar Request dan ID Proposal unik
      ...
      // Jika tidak jadi penguji DAN tidak jadi pembimbing, return kosong
      ...
      // 🔥 3. AMBIL DATA SEMINAR BERDASARKAN GABUNGAN ID 🔥
      // Kita gunakan .or() untuk mencari yang ID request-nya cocok (Penguji) 
      // ATAU ID proposal-nya cocok (Pembimbing)
      ...
      // 🔥 4. FORMAT DATA FINAL 🔥
      const mappedData = (requests || []).map((reqDetail: any) => {
        ...
        // Tentukan Label Role (Hanya untuk UI saja)
        let myRole = "Penguji";
          ...
          <div className="max-w-[1400px] mx-auto w-full"> 
            ...
            <div className="grid md:grid-cols-[1fr_280px] gap-8 mb-10 items-end">
              <div>
                ...
                <p className="text-slate-500 font-medium mt-1">Halaman khusus penilaian dan revisi seminar (Role: Kaprodi).</p>
              </div>
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {loading ? (
                      ...
                      <tr><td colSpan={5} className="py-20 text-center font-black text-slate-400">TIDAK ADA JADWAL MENGUJI/MENILAI</td></tr>
                      ...
                      <tr key={idx} className="group hover:bg-slate-50/80 transition-all cursor-default">
                        ...
                        <td className="py-8 px-10 text-center">
                          {/* 🔥 ROUTE SESUAI FOLDER KAPRODI 🔥 */}
                         <button 
  onClick={() => router.push(`/kaprodi/dashboardkaprodi/jadwalmengujiseminar?id=${item.id_request}`)}
