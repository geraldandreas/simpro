...
function DetailContent() {
  ...
  const [isRedirecting, setIsRedirecting] = useState(false); // 🔥 State baru untuk mencegah render saat pindah halaman
  ...
  useEffect(() => {
    if (id && isMounted && !isRedirecting) fetchDetailSeminar();
  }, [id, isMounted, isRedirecting]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 🔥 LOGIKA REDIRECT YANG AMAN DARI ERROR HYDRATION 🔥
      setIsRedirecting(true); // Mematikan render error screen
      router.push('/kaprodi/dashboardkaprodi/jadwalpengujiseminar'); 
      
    } catch (err: any) {
      alert("Gagal menyimpan: " + err.message);
      setSubmitting(false); // Hanya matikan loading jika gagal
    } 
  };

  // 🔥 MENCEGAH RENDER ERROR SAAT HYDRATION ATAU REDIRECT 🔥
