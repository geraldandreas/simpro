...
// 🔥 Interface yang kuat untuk StudentData agar tidak ada garis merah
interface StudentDataProps {
  ...
  proposal: {
    ...
    user: {
      nama: string;
      npm: string;
      avatar_url?: string | null; // avatar_url dideklarasikan di sini
    };
...
export default function PenjadwalanSeminarKaprodiClient() {
    ...
    const loadData = async () => {
      try {
       const { data: requestData, error: reqError } = await supabase
        ...
        // 🔥 Pemasangan data dilakukan dengan normalisasi agar selalu berbentuk single object
        ...
        const { data: dosenData } = await supabase
          .from('profiles')
          .select('id, nama')
          .in('role', ['dosen', 'kaprodi']) // 🔥 Filter dosen & kaprodi saja
          .order('nama');
  ...
  const getSupervisor = (role: string) => {
    ...
    const s = sups.find((item) => item.role === role);
    return s?.dosen?.nama || "-";
  };
  ...
  // Data sudah bersih dan pasti berbentuk object (bukan array) karena sudah dinormalisasi di atas
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="bg-white rounded-[2.5rem] border border-white shadow-xl shadow-slate-200/50 overflow-hidden relative group">
              ...
              <div className="p-10 flex items-center gap-8 relative z-10">
                ...
                {/* 🔥 LOGIKA RENDER AVATAR: TypeScript sudah aman di sini! */}
