...
interface ProposalItem {
  ...
  bidang: string;
  status: string;
  isRecommendedForMe?: boolean; // Apakah Kaprodi ini direkomendasikan jd pembimbing
  hasResponded?: boolean;        // Apakah Kaprodi sudah merespon perannya
  isRejectedByDosen?: boolean;   // Flag deteksi penolakan
  isReadyToAssign?: boolean;     // 🔥 TAMBAHAN LOGIKA SIAP DITETAPKAN
  user: {
    nama: string | null;
    npm: string | null;
...
export default function AksesProposalKaprodiPage() {
  ...
  const fetchProposals = async () => {
    try {
      ...
      const mappedData = (propData as any[]).map(p => {
        ...
        // 🔥 TAMBAHAN LOGIKA SIAP DITETAPKAN
        ...
        return {
          ...
          isReadyToAssign: ready // Masukkan ke state
        };
      });
  ...
  const filteredProposals = proposals.filter((item) => {
    ...
    const matchStatus =
      ...
      (filterStatus === "Siap Ditetapkan" && item.isReadyToAssign) || // 🔥 Filter Siap Ditetapkan
  ...
  return (
    <div className="flex min-h-screen bg-[#F8F9FB] font-sans text-slate-700">
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        {/* HEADER (Dipindahkan ke luar tag <main> dan ditata ulang) */}
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-10 sticky top-0 z-20 shrink-0">
          <div className="flex items-center gap-6">
            <div className="relative w-72 group">
              {/* Ruang kosong kiri (bisa untuk judul halaman jika perlu) */}
            </div>
          </div>
