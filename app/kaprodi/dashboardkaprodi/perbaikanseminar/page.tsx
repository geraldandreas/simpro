...
export default function PerbaikanSeminarKaprodiList() {
  ...
  const fetchDaftarRevisi = async () => {
    try {
      ...
      // Ambil seluruh data feedback revisi milik Kaprodi ini
        ...
        <div className="grid md:grid-cols-[1fr_280px] gap-8 mb-10 items-end">
          <div>
            ...
            <p className="text-slate-500 font-medium mt-1">
              Pantau proses revisi mahasiswa dan berikan persetujuan (Role: Kaprodi).
            </p>
          </div>
                  ...
                  filteredList.map((mhs) => {
                    ...
                    return (
                      <tr key={mhs.request_id} className="group hover:bg-slate-50/80 transition-all">
                        ...
                        {/* AKSI */}
                        <td className="py-8 px-10 text-center">
                          {/* 🔥 ROUTE DIARAHKAN KE FOLDER DETAIL MILIK KAPRODI 🔥 */}
                          <button 
                            onClick={() => router.push(`/kaprodi/dashboardkaprodi/detailperbaikanseminar?id=${mhs.request_id}`)}
