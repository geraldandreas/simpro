...
export default function DokumenSidangMahasiswa() {
  ...
  const handleAjukanJadwal = async () => {
    ...
    try {
      ...
      if (kaprodi) {
        await sendNotification(kaprodi.id, "Pengajuan Sidang Baru", "Mahasiswa telah menyelesaikan pemberkasan sidang dan siap dijadwalkan.");
      }
  ...
  return (
    <div className="flex h-screen bg-[#F4F7FE] font-sans text-slate-700 overflow-hidden">
      <Sidebar />
      <main className="flex-1 ml-64 flex flex-col h-full overflow-hidden bg-[#F8F9FB]">
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-end px-10 shrink-0 z-20">
        ...
        <div className="flex-1 overflow-y-auto p-10 custom-scrollbar">
          <div className="max-w-6xl mx-auto w-full">
            <div className="mb-10">
               <h1 className="text-3xl font-black text-slate-800 tracking-tight uppercase">Pemberkasan Sidang</h1>
                  ...
                  <div className="p-8 space-y-4">
                      ...
                      return (
                        <div key={doc.id} className={`flex items-center justify-between p-6 rounded-[2rem] border transition-all duration-300 ${isComplete ? 'bg-emerald-50/40 border-emerald-100' : isPending ? 'bg-blue-50/30 border-blue-100' : 'bg-white border-slate-100 hover:border-slate-200'}`}>
                          <div className="flex items-center gap-6">
                            ...
                            <div>
                              <p className="text-sm font-black text-slate-800 uppercase tracking-tight leading-none mb-1.5">{doc.label}</p>
                              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{doc.sub}</p>
                            </div>
                          </div>
                    ...
                    <div className="mt-8 p-6 bg-blue-900 rounded-[2rem] text-white flex items-center gap-6 shadow-xl shadow-blue-100">
                      ...
                      <div className="flex-1">
                        <h4 className="text-sm font-black uppercase tracking-tight">Pengiriman Soft File Tambahan</h4>
