...
export default function UploadProposalMahasiswaClient() {
  ...
  const isDosenSubmitted = data?.recData && data.recData.length > 0;
  ...
  const handleSubmit = async () => {
    if (!activeProposal || (!fileToUpload && !data?.proposalFormatted)) return;
    ...
    try {
      ...
      if (kaprodi) {
        const mhsNama = user?.user_metadata?.full_name || user?.email || "Seorang mahasiswa";
  ...
  const handleAjukanDosen = async () => {
    const proposalId = data?.proposalFormatted?.id;
    if (!proposalId) return alert("Submit proposal terlebih dahulu");
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const proposalId = data?.proposalFormatted?.id;
      const storagePath = data?.proposalFormatted?.storagePath;

      if (proposalId) {
        if (storagePath) {
          await supabase.storage.from("proposals").remove([storagePath]);
        }
        await supabase.from("proposals").delete().eq("id", proposalId).eq("user_id", user.id);
      }
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                ...
                {isUploaded && activeProposal && (
                  <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
                    <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 font-medium">{activeProposal.id ? "Sudah tersimpan di server" : "Draft (Belum disimpan)"}</p>
                        </div>
                      </div>
                    ...
                    <div className="bg-white p-2 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden">
                      ...
                      {activePreviewUrl ? (
                        <iframe src={activePreviewUrl} className="w-full h-[700px] border-none" />
                      ) : (
              ...
              <div className="lg:col-span-4 space-y-8">
                    ...
                    <div className="space-y-5">
                      ...
                      <div>
                         ...
                         <span className={`inline-flex px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider
                          ${activeProposal.id ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"}`}>
                ...
                <section className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/40">
                  <h2 className="text-lg font-black text-slate-800 mb-8 flex items-center gap-2 tracking-tighter">
                  ...
                  <div className="space-y-6">
                    <div className="space-y-2">
                      ...
                      <textarea
                        ...
                        disabled={activeProposal?.is_locked}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative group">
                        ...
                        {!activeProposal?.is_locked && <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none transition-colors" size={18} />}
                      </div>
                    </div>
                      ...
                      <div key={p.id} className="space-y-2">
                        ...
                        <div className="relative group">
                          <select
                            ...
                            disabled={isDosenSubmitted || activeProposal?.is_locked}
                          ...
                          {(!isDosenSubmitted && !activeProposal?.is_locked) && <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" size={18} />}
                        </div>
                      </div>
                    ))}
                    ...
                    <button
                      ...
                      disabled={!(data?.proposalFormatted as any)?.id || loading || isDosenSubmitted}
